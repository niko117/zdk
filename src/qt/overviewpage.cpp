...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::NXBoost)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("NXBoost")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNXBoostAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNXBoostAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeNXBoostAmount = strAnonymizeNXBoostAmount.remove(strAnonymizeNXBoostAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNXBoostAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeNXBoostAmount * COIN) nMaxToAnonymize = nAnonymizeNXBoostAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeNXBoostAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeNXBoostAmount));
        strAnonymizeNXBoostAmount = strAnonymizeNXBoostAmount.remove(strAnonymizeNXBoostAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNXBoostAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
